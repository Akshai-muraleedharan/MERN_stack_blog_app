  ...
  return (
    <div className='flex flex-col p-4 rounded-xl shadow-sm w-full md:w-[80%] my-2'>
      {checkComment.map((item) => (
        <ul key={item._id}>
          <li className='my-4'>
            <p className='flex dark:text-white text-primary-text font-semibold gap-1 place-items-center'>{item.username} <FaCircleDot className="text-[6px]" /> <span className='text-gray-400 text-xs font-medium' >{dateConvert(item.createdAt.slice(5, 10))}</span></p>
            <p className='text-lg dark:text-white my-3 text-primary-text'>{item.comment}</p>

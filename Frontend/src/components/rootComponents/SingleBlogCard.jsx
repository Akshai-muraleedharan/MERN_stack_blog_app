      ...
      <InfiniteScroll
        ...
        {data.filter(item => item.published === true).map((item) => (
          <Link to={user ? `/blog/auth/${item._id}` : `/blog/${item._id}`} key={item._id}>
            <div className="card dark:bg-dark-cards-bg bg-card_bg  w-[100%] card-sm shadow-md mb-5 cursor-pointer dark:border dark:border-dark-borders-color">
              <div className="card-body">
                ...
                <div className="flex justify-between items-center">
                  <div className='flex gap-4'>
                    <span className="inline-flex justify-center dark:text-dark-smalls-text text-base_text items-center flex-wrap py-1 px-3 font-semibold border rounded-md dark:border-dark-borders-color border-gray-200 ">
                      {`Like ${item.likes}`}
                    </span>
                    <span className="inline-flex justify-center dark:text-dark-smalls-text text-base_text items-center flex-wrap py-1  px-3 font-semibold border rounded-md dark:border-dark-borders-color border-gray-200">
                  ...
                  <span className='dark:text-dark-smalls-text text-base_text'>{dateConvert(item.createdAt.slice(5, 10))}</span>
                </div>
              </div>
            </div>
          </Link>
